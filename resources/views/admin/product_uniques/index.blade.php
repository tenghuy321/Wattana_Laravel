...
@section('content')
    <div class="" x-data="reorderTable()" x-init="initSortable()">
        <div class="my-2 px-2 md:px-4 flex items-center justify-between">
            <a href="{{ route('product_unique.create') }}"
                ...
                <table class="min-w-full table-fixed">
                    <thead class="text-black sticky top-0 z-10">
                        <tr>
                            ...
                            <th class="text-left py-3 px-4 text-xs border-r border-[#FF3217] w-[200px]">Title</th>
                            <th class="text-left py-3 px-4 text-xs border-r border-[#FF3217] w-[200px]">Action</th>
                        </tr>
                    </thead>
                    <tbody x-ref="tableBody" class="text-gray-700">
                        @foreach ($product_uniques as $index => $product_unique)
                            <tr class="border-t border-[#FF3217] cursor-move" draggable="true" x-bind:data-id="{{ $product_unique->id }}"
                                @dragstart="dragStart($event, {{ $product_unique->id }})" @dragover.prevent
                                @drop="drop($event, {{ $product_unique->id }})">
                                <td class="py-3 px-4 text-xs max-w-[200px] border-r border-[#FF3217]">
                                    <div class="flex items-center h-full w-full">{{ $product_unique->order }}</div>
                                </td>
                                <td class="py-3 px-4 text-xs max-w-[200px] border-r border-[#FF3217]">
                                    <div class="flex flex-col truncate">
                                        <p>{{ $product_unique->title['en'] ?? '' }}</p>
                                        <p>{{ $product_unique->title['kh'] ?? '' }}</p>
                                    </div>
                                </td>
                                <td class="py-3 px-4 text-xs border-r border-[#FF3217]">
                                    <div class="flex items-center h-full space-x-2 w-full">
                                        <a href="{{ route('product_unique.delete', $product_unique->id) }}" title="Delete"
                                            onclick="event.preventDefault(); deleteRecord('{{ route('product_unique.delete', $product_unique->id) }}')">
                                        ...
                                        <a href="{{ route('product_unique.edit', $product_unique->id) }}" title="Edit">
                    ...
                    this.$nextTick(() => {
                        ...
                        new Sortable(tableBody, {
                            ...
                            onEnd: async (event) => {
                                ...
                                let response = await fetch("{{ route('product_unique.reorder') }}", {
                                    method: "POST",
                                    headers: {
