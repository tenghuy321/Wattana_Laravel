...
@section('content')
    <div class="" x-data="reorderTable()" x-init="initSortable()">
        <div class="my-2 px-2 md:px-4 flex items-center justify-between">
            <a href="{{ route('hero_banner.create') }}"
                ...
                <table class="min-w-full table-fixed">
                    ...
                    <tbody x-ref="tableBody" class="text-gray-700">
                        @foreach ($heroes as $index => $hero)
                            <tr class="border-t border-[#FF3217] cursor-move" draggable="true" x-bind:data-id="{{ $hero->id }}"
                                ...
                                <td class="text-left py-3 px-4 text-[10px] md:text-[12px] border-r border-[#FF3217]">
                                    <div class="flex items-center h-full w-full">
                                        <img src="{{ asset($hero->image) }}" alt="" class="w-20 h-12 object-cover object-center">
                                    </div>
                                </td>
                                <td class="py-3 px-4 text-xs border-r border-[#FF3217]">
                                    <div class="flex items-center h-full space-x-2 w-full">
                                        <a href="{{ route('hero_banner.delete', $hero->id) }}" title="Delete"
                                            onclick="event.preventDefault(); deleteRecord('{{ route('hero_banner.delete', $hero->id) }}')">
                                        ...
                                        <a href="{{ route('hero_banner.edit', $hero->id) }}" title="Edit">
                    ...
                    this.$nextTick(() => {
                        ...
                        new Sortable(tableBody, {
                            ...
                            onEnd: async (event) => {
                                ...
                                let response = await fetch("{{ route('hero_banner.reorder') }}", {
                                    method: "POST",
                                    headers: {
