<x-app-layout>
    <div class="max-w-7xl mx-auto bg-white shadow-md rounded-lg p-6 my-2">
        ...
        <form action="{{ route('nav.update', $navbar->id) }}" method="POST" enctype="multipart/form-data"
            class="space-y-4">
            @csrf
            @method('PATCH')
                ...
                @if (!empty($navbar->title['en']))
                    <div class="p-0 space-y-4">
                        ...
                        <div>
                            ...
                            <input value="{{ old('title.en', $navbar->title['en'] ?? '') }}" type="text"
                ...
                @if (!empty($navbar->title['kh']))
                    <div class="p-0 space-y-4">
                        ...
                        <div>
                            ...
                            <input value="{{ old('title.kh', $navbar->title['kh'] ?? '') }}" type="text"
            ...
            <!-- Dropzone for Image -->
            @if (!empty($navbar->image))
                <div>
                    <label for="dropzone-file{{ $navbar->id }}" id="drop-area"
                        ...
                        <div class="flex flex-col items-center justify-center pt-5 pb-6 w-full h-full bg-contain bg-center bg-no-repeat rounded-md text-center"
                            id="img-preview" style="background-image: url({{ asset($navbar->image) }})">
                        ...
                        <input id="dropzone-file{{ $navbar->id }}" type="file" class="hidden" name="image"
