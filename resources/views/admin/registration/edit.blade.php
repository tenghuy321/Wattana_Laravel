<x-app-layout>
    <div class="max-w-4xl mx-auto bg-white shadow-md rounded-lg p-6 my-2">
        <h2 class="text-2xl font-bold">Edit Registration</h2>
        <form action="{{ route('registration.update', $registration->id) }}" method="POST" enctype="multipart/form-data"
            ...
            <!-- Dropzone for Image -->
            <div>
                <label for="dropzone-file{{ $registration->id }}" id="drop-area"
                    ...
                    <div class="flex flex-col items-center justify-center pt-5 pb-6 w-full h-full bg-contain bg-center bg-no-repeat rounded-md text-center"
                        id="img-preview" style="background-image: url({{ asset($registration->image) }})">
                    ...
                    <input id="dropzone-file{{ $registration->id }}" type="file" class="hidden" name="image" accept="image/*"
